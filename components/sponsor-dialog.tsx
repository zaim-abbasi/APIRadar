...
const WALLETS = {
  TRX: {
    ...
    icon: Wallet,
    warning: "Only send USDT (TRC20) to this address. Sending other assets like TRX may result in permanent loss."
  },
  BTC: {
    name: "Bitcoin",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[380px] bg-card/60 backdrop-blur-sm border-border/50 p-0 gap-0 shadow-lg overflow-hidden">
        <DialogHeader className="p-5 pb-0 text-center">
          <DialogTitle className="text-base font-semibold tracking-tight">Sponsor APIRadar</DialogTitle>
        ...
        <div className="px-5 pb-5 pt-4">
          <Tabs defaultValue="TRX" className="w-full">
            ...
            {Object.entries(WALLETS).map(([key, wallet]) => (
              <TabsContent key={key} value={key} className="mt-0 focus-visible:outline-none animate-in fade-in-50 slide-in-from-bottom-1 duration-200">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="w-full space-y-1.5">
                    <div className="flex items-center justify-between text-[11px] text-muted-foreground px-1">
                      ...
                      <span className={cn("inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-muted/40 border border-border/20 font-medium", wallet.color)}>
