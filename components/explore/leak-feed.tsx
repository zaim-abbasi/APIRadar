...
const FeedItem = React.memo(
  ({
    ...
    return (
      <div className="block px-3.5 py-2.5 sm:px-5 sm:py-2 sm:hover:bg-white/[0.03] transition-colors duration-150 group/item animate-fade-in-up opacity-0"
        style={{ animationDelay: `${Math.min(index * 40, 300)}ms` }}
      >
        <div className="flex items-start sm:items-center gap-3 sm:gap-4">
          {/* Status dot */}
          <div className="mt-1.5 sm:mt-0 flex-shrink-0">
            <div
              className={cn(
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-4 flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between sm:justify-start gap-2 sm:gap-3 flex-shrink-0 sm:w-[260px] lg:w-[260px] xl:w-[280px]">
              {/* Redacted key */}
              <code className="text-[11px] sm:text-sm font-mono bg-muted/40 px-1.5 sm:px-2.5 py-0.5 rounded-md border border-border/40 text-foreground transition-colors duration-150 truncate min-w-0 tracking-tight">
              ...
              <div className="flex sm:hidden items-center gap-2 flex-shrink-0">
                <div className="flex items-center justify-center gap-1 bg-red-500/10 text-red-500 border border-red-500/20 px-1.5 py-0.5 rounded text-[9px] uppercase font-bold tracking-wider">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2.5 text-[11px] sm:text-sm text-muted-foreground/80 min-w-0 flex-1">
              <FolderGit2 className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0 text-coral/80" />
              ...
              <FileCode2 className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0 opacity-70" />
          ...
          <div className="hidden sm:flex items-center gap-3 flex-shrink-0 sm:ml-auto">
            <div className="w-[90px] items-center justify-center gap-1.5 flex bg-red-500/10 text-red-500 border border-red-500/20 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-widest flex-shrink-0">
              <ShieldAlert className="h-3 w-3" />
              <span>Exposed</span>
            </div>
            <span className="text-border flex-shrink-0 text-muted-foreground/30">·</span>
            <span className="text-xs text-muted-foreground/70 whitespace-nowrap flex-shrink-0 tabular-nums font-medium inline-flex items-center gap-1.5 sm:w-[90px] justify-start">
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain custom-scrollbar min-h-0" style={{ WebkitOverflowScrolling: 'touch' }}>
              {isLoading && validLeaks.length === 0 ? (
                ...
                {showSignInPrompt && onSignIn && (
                  <div 
                    onClick={onSignIn}
                    className="block px-3.5 py-3 sm:px-5 sm:py-2.5 sm:hover:bg-white/[0.03] transition-colors duration-150 cursor-pointer group animate-fade-in-up border-t border-border/10"
                  >
                    <div className="flex items-start sm:items-center gap-3 sm:gap-4">
                      {/* Lock Icon */}
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex flex-col gap-1 flex-1 min-w-0">
                          <div className="flex items-center gap-2 sm:gap-3">
                             <span className="inline-flex items-center justify-center rounded-md px-1.5 py-0.5 text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-coral/10 text-coral border border-coral/20">
                               RADAR_PRO
                             </span>
                             <span className="text-[10px] sm:text-xs font-bold text-muted-foreground/60 uppercase tracking-tight truncate">
                               {isRestricted ? "Access Restricted" : "Intelligence Limit Reached"}
                             </span>
                          </div>
                          
                          <p className="text-[11px] sm:text-sm text-foreground/70 font-medium truncate">
                            Sign in with Google to unlock full intelligence.
                          </p>
                        </div>
