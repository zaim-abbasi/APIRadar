            ...
            <div className="flex flex-col h-full pr-8 sm:pr-0">
              <div className="flex-1 flex flex-col gap-2 sm:gap-2.5 w-full">
                ...
                <div className="flex flex-col gap-1.5 sm:gap-2 text-xs sm:text-sm">
                  {(() => {
                    const parsed = safeRepoUrl
                      ? parseGitHubRepoUrl(safeRepoUrl)
                      : null;
                    if (!parsed || !safeRepoUrl)
                      return (
                    ...
                    return (
                      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-2">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <span className="text-xs sm:text-sm text-coral font-medium whitespace-nowrap flex-shrink-0">
                            Repo Name:
                          </span>
                          <a
                            href={
                              safeFilePath
                                ? `${safeRepoUrl}/blob/HEAD/${safeFilePath}`
                                : safeRepoUrl
                            }
                            target="_blank"
                        ...
                        <span className="hidden sm:inline text-muted-foreground/70 text-sm">
                          by
                        </span>
                        <a
                ...
                {safeFilePath && (
                  <div className="flex items-start sm:items-center gap-1.5 sm:gap-2 min-w-0">
                    ...
                    <code
                      className="text-xs sm:text-sm break-all sm:break-words sm:whitespace-normal bg-muted/50 px-1.5 py-0.5 rounded border border-border/30 inline-block"
                      title={safeFilePath}
                    >
                      {safeFilePath}
                    </code>
      ...
      <div className="relative">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-5">
          {validLeaks.map((leak, index) => (
            <div key={leak.id || `leak-${index}`} className={cn("relative")}>
              <LeakCard leak={leak} index={index} />
            </div>
          ))}
        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-4">
                  <div className="flex-1 space-y-1.5 sm:space-y-2 min-w-0">
                    ...
                    <div className="text-xs sm:text-sm text-muted-foreground/90 leading-relaxed">
                      You are seeing 4 of 19,000+ active leaks. Sign in for full
                      access to the live feed and repository details.
                    </div>
