...
export default function UpgradeToProCardWithTrialButton({
  session,
  onlyButton = false,
  buttonClassName,
}: UpgradeToProCardWithTrialButtonProps) {
  const { plan, requestedTrial: hookRequestedTrial, refresh } = usePlanCheck();
  const [requestedTrial, setRequestedTrial] = React.useState<boolean>(
    !!session?.user?.requestedTrial
  );
  ...
  const button =
    isBasic && !requestedTrial ? (
      ...
      <span className="inline-block px-3 py-1 rounded bg-muted text-muted-foreground font-medium text-xs sm:text-sm">
        Pro trial request sent
      </span>
    ) : null;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 min-h-[80px]">
            ...
            <div className="flex flex-col gap-1 flex-1 min-w-0 text-left">
              ...
              <span className="text-muted-foreground text-xs sm:text-sm">
                Get Full Access to All API Key Leaks and Advanced Features for a
                Limited Time.
              </span>
            </div>
